# Package index

## Plot Functions

Functions that return ggplot2 objects ready to display or customize. All
calculations run in the database.

- [`dbplot_histogram()`](https://edgararuiz.github.io/dbplot/reference/dbplot_histogram.md)
  : Histogram
- [`dbplot_bar()`](https://edgararuiz.github.io/dbplot/reference/dbplot_bar.md)
  : Bar plot
- [`dbplot_line()`](https://edgararuiz.github.io/dbplot/reference/dbplot_line.md)
  : Line plot
- [`dbplot_raster()`](https://edgararuiz.github.io/dbplot/reference/dbplot_raster.md)
  : Raster plot
- [`dbplot_boxplot()`](https://edgararuiz.github.io/dbplot/reference/dbplot_boxplot.md)
  : Boxplot

## Computation Functions

Functions that return data frames with aggregated data. Use these when
you need custom plots or further data processing.

- [`db_compute_bins()`](https://edgararuiz.github.io/dbplot/reference/db_compute_bins.md)
  : Calculate histogram bins
- [`db_compute_count()`](https://edgararuiz.github.io/dbplot/reference/db_compute_count.md)
  : Aggregates over a discrete field
- [`db_compute_raster()`](https://edgararuiz.github.io/dbplot/reference/db_compute_raster.md)
  [`db_compute_raster2()`](https://edgararuiz.github.io/dbplot/reference/db_compute_raster.md)
  : Aggregates intersections of two variables
- [`db_compute_boxplot()`](https://edgararuiz.github.io/dbplot/reference/db_compute_boxplot.md)
  : Returns a dataframe with boxplot calculations

## Utility Functions

Building blocks for creating custom database queries with binning.

- [`db_bin()`](https://edgararuiz.github.io/dbplot/reference/db_bin.md)
  : Bin formula

## Package Documentation

- [`dbplot-package`](https://edgararuiz.github.io/dbplot/reference/dbplot-package.md)
  [`dbplot`](https://edgararuiz.github.io/dbplot/reference/dbplot-package.md)
  : dbplot: Simplifies Plotting Data Inside Databases
